...
def process_gp(file1, file2):
    ...
    df1["SALES PROGRAM"] = df1.filter(regex='(?i)rs subsidi|special early bird|special price').sum(axis=1)
    ...
    df1['SURAT JALAN EV'] = df1.filter(regex='(?i)surat jalan ev| surat jalan cloud|surat jalan januari').sum(axis=1)
    ...
    df1['AKSESORIS LAIN-LAIN'] = df1.filter(regex='(?i)poles|repair|shopping voucher|electricity voucher|gasoline voucher|shoping voucher').sum(axis=1)
    ...
    df1['VOUCHER EV & GIIAS, CASH DISKON/ trade in, EX Factory GAP'] = df1.filter(regex='(?i)electricity voucher|gasoline voucher|conversion|special kredit package|regional').sum(axis=1)
